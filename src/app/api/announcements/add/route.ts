...
try {
  ...
  Subscriber = mongoose.models.Subscriber || mongoose.model('Subscriber', tempSchema);
} catch (error) {
...
async function sendEmailReceipt(transporter, subscriber, announcement) {
  try {
    ...
    if (!recipientEmail) {
      ...
      return { success: false, email: "unknown", error: "No email address found" };
    }

    const info = await transporter.sendMail({
      from: `"${process.env.EMAIL_SENDER_NAME || 'MNSTS Announcements'}" <${process.env.EMAIL_USER}>`,
      to: recipientEmail,
    ...
    console.error(`Error sending announcement email to ${subscriber.email || 'unknown'}:`, error);
    return { success: false, email: subscriber.email || 'unknown', error: (error as Error).message };
  }
}
...
async function sendBatchEmails(announcement) {
  ...
  try {
    ...
    const results = await Promise.allSettled(
      subscribers.map(subscriber => sendEmailReceipt(transporter, subscriber, announcement))
    );
