...
async function sendEmailReceipt(transporter, subscriber, event) {
  try {
    ...
    const info = await transporter.sendMail({
      from: `"${process.env.EMAIL_SENDER_NAME || 'MNSTS Events'}" <${process.env.EMAIL_USER}>`,
      to: recipientEmail,
    ...
    console.error(`Error sending event email to ${subscriber.email || 'unknown'}:`, error);
...
async function sendBatchEmails(event) {
  ...
  try {
    ...
    const results = await Promise.allSettled(
      subscribers.map(subscriber => sendEmailReceipt(transporter, subscriber, event))
    );
